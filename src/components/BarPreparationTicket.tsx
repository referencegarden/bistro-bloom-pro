...
interface BarPreparationTicketProps {
  orderId: string;
}
...
export function BarPreparationTicket({ orderId }: BarPreparationTicketProps) {
  ...
  const loadTicketData = async () => {
    try {
      ...
      console.error("Error loading bar ticket:", error);
    }
  };
  ...
  return (
    <div className="bar-ticket preparation-ticket bg-white text-black p-4 max-w-[80mm] mx-auto font-mono" data-ticket-type="bar">
      <style>{`
        @media print {
          body * {
            visibility: hidden;
