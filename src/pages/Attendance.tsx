...
export default function Attendance() {
  ...
  const [wifiStatus, setWifiStatus] = useState<WifiStatus>({ isConnected: false, ssid: null, ipAddress: null });
    ...
    try {
      if (actionType === "check_in") {
        ...
        toast({
          ...
          description: `Vous avez pointé à ${format(new Date(), "HH:mm", { locale: fr })}`
        });
      } else {
        const { error } = await supabase
          .from("attendance")
        ...
        toast({
          ...
          description: `Départ enregistré à ${format(new Date(), "HH:mm", { locale: fr })}`
        });
      }
    ...
    <div className="container mx-auto p-6 max-w-4xl">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-success">
                    {format(new Date(todayAttendance.check_in_time), "HH:mm", { locale: fr })}
                  </p>
                </div>
              </div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {format(new Date(todayAttendance.check_out_time), "HH:mm", { locale: fr })}
                    </p>
                  </div>
                </div>
              )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <Button
          ...
          disabled={!canCheckIn || !wifiStatus.isConnected || loading}
        ...
        <Button
          ...
          disabled={!canCheckOut || !wifiStatus.isConnected || loading}
          variant="outline"
          className="h-20 text-lg"
        >
        ...
        <CardContent>
          <div className="space-y-2">
            {weeklyAttendance.length > 0 ? (
                ...
                >
                  <div>
                    <p className="font-medium">
                      {format(new Date(record.date), "EEEE d MMMM", { locale: fr })}
                    </p>
                    <p className="text-sm text-muted-foreground">
              ...
              <p className="text-center text-muted-foreground py-4">Aucun historique cette semaine</p>
            )}
          </div>
        </CardContent>
      </Card>
